    ...
    Scaffold { padding ->
        Column(
            ...
            Row(
                ...
                Text("Create Schedule", fontWeight = FontWeight.Bold, fontSize = 20.sp)
            ...
            PrimaryButton(
                text = "Create",
                onClick = {
                    val parsedDate = runCatching { LocalDate.parse(date) }.getOrNull() ?: LocalDate.now()
                    if (name.isNotBlank() && startTime.isNotBlank() && endTime.isNotBlank()) {
                        scheduleViewModel.addSchedule(name, category, parsedDate, startTime, endTime)
                        onCreated()
                    }
                }
            )
